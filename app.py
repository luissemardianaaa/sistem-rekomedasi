...
def load_ratings():
    if os.path.exists("user_ratings.csv"):
        # Use encoding='utf-8-sig' and auto-separator detection
        # Added index_col=False for consistency
        df = pd.read_csv("user_ratings.csv", sep=None, engine='python', encoding='utf-8-sig', index_col=False)
        df.columns = df.columns.str.strip()
        if not df.empty:
            df["user_id"] = df["user_id"].astype(str)
            df["game_id"] = df["game_id"].astype(str)
...
def save_rating(user_id, game_id, score, ratings_df, game_name=None):
    user_id = str(user_id)
    game_id = str(game_id)
...
def rating_component(row, user_id, ratings_df, key_prefix):
    ...
    with col2:
        rate = st.number_input(
            ...
            key=f"{key_prefix}_{row['game_id']}"
        )
        
        if st.button("Simpan", key=f"btn_{key_prefix}_{row['game_id']}"):
            display_name = row.get('name') if pd.notna(row.get('name')) else row.get('game_name')
            save_rating(user_id, row["game_id"], rate, ratings_df, display_name)
            st.success(f"Rating {display_name} disimpan!")
            st.rerun()
